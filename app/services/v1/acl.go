...
func (acl_s *ACLService) CreateAccessRelationship(f_user_id string, token *string, rb validators.ACLPostBody) error {
	...
	if token != nil {
		...
		_, err_get_doc_parent := acl_s.Model.GetAccessRelationship(nil, rb.ParentMemberID, rb.ParentMemberID)
		if err_get_doc_parent != nil {
			return err_get_doc_parent
		}
		_, err_get_doc_head_parent := acl_s.Model.GetAccessRelationship(nil, f_user_id, rb.ParentMemberID)
...
func (acl_s *ACLService) UpdateAccessRelationship(f_head_user_id string, rb validators.ACLPutBody) error {
	// Get Access relation
	doc, err := acl_s.Model.GetAccessRelationship(&f_head_user_id, rb.ParentMemberID, rb.Access.ChildMemberId)
	if err != nil {
